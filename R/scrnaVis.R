      ...
      tabItem(
        ...
        column(
          ...
          box(
            ...
            br(),
            plotOutput("FeaturePlot", width = "80%", height =
                         "400px")
          )
        ))
      ),
      ...
      tabItem(
        ...
        box(
          ...
          div(
            ...
            selectInput("select_geneset_db", "Gene Sets Categories",
                        choices = getsetdb)
          ),
          div(
            style = "display: inline-block;vertical-align:top; width: 34%;",
            selectInput("select_geneset_sub_db", "Gene Sets Sub Categories",
                        choices = NULL)
          ),
          br(),
          div(
        ...
        observeEvent(input$start, {
          ...
          if (is.null(object@assays$SCT)) {
            ...
            heatmap <- ComplexHeatmap::pheatmap(
              ...
              color = c(
                ...
                colorRampPalette(colors = c("#f7fbff", "#b2182b"))(length(bk) /
                                                                     1)
              ),
              breaks = bk,
              scale = "row",
              legend_breaks = seq(-1, 2, 1),
              name = "Exp"
            )
          ...
          observe({
            ...
            if (x %in% c("H", "C8")) {
              updateSelectInput(
                session,
                "select_geneset_sub_db",
                choices = NULL,
                selected = NULL
              )
            } else if (x == "C2") {
              y = "CP:KEGG"
              updateSelectInput(session, "select_geneset_sub_db",
                                choices = y)
            } else if (x == "C5") {
              y = "BP"
              updateSelectInput(session, "select_geneset_sub_db",
